...
**Screenshots are NOT committed.** They are written under `build/` and regenerated by
`.github/workflows/screenshots.yml`, which records both the base branch and the branch on one
runner and posts the visual difference as a PR comment. Recording locally is for looking at them
yourself; nothing you record is read by anything else, so there is no need to re-record before
pushing and no reason to add them to a commit.
...
version of a binary for ever. The reg-actions comment shows before and after side by side, which is
what a reviewer wants; two opaque PNG blobs in a diff cannot be compared by eye anyway.
